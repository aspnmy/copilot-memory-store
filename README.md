...
# 编译后的可执行文件位于 target/release/memory-store.exe (Windows)
# 或 target/release/memory-store (Linux/macOS)
```
...
```bash
# 使用默认记忆文件路径（.copilot-memory.json）
memory-store

# 指定自定义记忆文件路径
memory-store /path/to/memory.json
```
