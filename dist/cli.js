 ...
 */
async function cmdCompress(records, p) {
    const query = typeof p.opts.query === "string" ? p.opts.query : p.args.join(" ").trim();
    if (!query) {
        console.log("❌ compress requires --query <text> (or positional query).");
        return;
    }
...
async function cmdDelete(p) {
    ...
    if (!id) {
        console.log("❌ delete requires an id.");
        return;
    }
    ...
    try {
        switch (p.cmd) {
            case "add":
                ...
                break;
            case "search":
                cmdSearch(state.records, p);
                break;
            case "compress":
                await cmdCompress(state.records, p);
                break;
            case "delete":
                await cmdDelete(p);
                break;
            case "purge":
                ...
                break;
            case "export":
                cmdExport(state.records);
                break;
            case "stats":
                cmdStats(state.records);
                break;
            default:
...
export async function main() {
    ...
    const active = loaded.records.filter((r) => !r.deletedAt).length;
