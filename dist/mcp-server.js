 ...
 */
const server = new McpServer({ name: "copilot-memory-store", version: "0.3.0" }, {
    capabilities: {},
    instructions: `
        ...
        try {
            ...
            if (existingTags.length >= 2) {
                ...
                if (result.action === "accept" && result.content) {
                    const selected = result.content.selectedTags || [];
                    const custom = result.content.customTag;
                    tags = [...selected];
 ...
 */
server.registerResource("stats", "memory://stats", {
 ...
 */
server.registerResource("recent", "memory://recent", {
 ...
 */
server.registerPrompt("summarize-memories", {
 ...
 */
server.registerPrompt("remember-decision", {
 ...
 */
server.registerPrompt("inject-context", {
