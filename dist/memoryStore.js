...
function resolveLockPath(memoryPath) {
    const envLock = process.env.MEMORY_LOCK_PATH?.trim();
    if (envLock)
        return path.isAbsolute(envLock) ? envLock : path.resolve(process.cwd(), envLock);
 ...
 */
async function acquireLock(lockPath, timeoutMs = 2500) {
...
function readJsonArray(filePath) {
    ...
    if (!Array.isArray(data))
        throw new Error(`Memory file must be a JSON array: ${filePath}`);
    return data;
}
 ...
 */
export function loadStore(memoryPath) {
...
export async function addMemory(opts) {
    ...
    try {
        ...
        if (!t)
            throw new Error("Cannot add an empty memory.");
 ...
 */
export async function softDeleteById(opts) {
    const mp = resolveMemoryPath(opts.memoryPath);
    const lock = resolveLockPath(mp);
    await acquireLock(lock);
    try {
...
export async function purge(opts) {
    ...
    try {
        ...
        if (id)
            ...
            predicate = (r) => r.tags.map((t) => t.toLowerCase()).includes(tag);
        else {
...
function scoreRecord(r, query) {
    ...
    for (const tok of tokens) {
        ...
        score += hits * 5;
        // Tag match bonus
        if (r.tags.some((t) => t.toLowerCase() === tok))
            score += 8;
        // Keyword match bonus (extracted keywords are pre-indexed)
        if (r.keywords?.some((k) => k === tok))
            score += 6;
    }
 ...
 */
export function formatSearchResults(hits, query) {
    if (hits.length === 0) {
    ...
    for (let i = 0; i < hits.length; i++) {
        ...
        const meta = [];
        if (h.tags.length)
            meta.push(`Tags: ${h.tags.join(", ")}`);
        if (h.keywords.length)
            meta.push(`Keywords: ${h.keywords.slice(0, 5).join(", ")}`);
 ...
 */
export function compressDeterministic(opts) {
    ...
    const md = lines.join("\n") + "\n";
    if (md.length <= budget)
        return { markdown: md, included: hits, budget, used: md.length };
    const out = [];
    let size = 0;
